...
def teacher_dashboard():
    ...
    with tab2:
        ...
        if not st.session_state.student_answers:
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                avg_score = sum([ans['점수'] for ans in st.session_state.student_answers]) / len(st.session_state.student_answers) if st.session_state.student_answers else 0
                st.metric("평균 점수", f"{avg_score:.1f}점")
            with col2:
                correct_count = sum([1 for ans in st.session_state.student_answers if ans['점수'] == 100])
                correct_rate = (correct_count / len(st.session_state.student_answers)) * 100 if st.session_state.student_answers else 0
...
def student_portal():
    ...
    with tab2:
        ...
        if not my_answers:
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                avg_score = sum([ans['점수'] for ans in my_answers]) / len(my_answers) if my_answers else 0
                st.metric("평균 점수", f"{avg_score:.1f}점")
            with col2:
                correct_count = sum([1 for ans in my_answers if ans['점수'] == 100])
                correct_rate = (correct_count / len(my_answers)) * 100 if my_answers else 0
                st.metric("정답률", f"{correct_rate:.1f}%")
            with col3:
